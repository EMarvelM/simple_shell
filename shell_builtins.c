...
/**
 * exiting_program - exits the shell with an optional status
 * @cmd_tkn: command entered
 *
 * Return: void
 */
void exiting_program(char **cmd_tkn)
{
    if ((shell_isatty(STDIN_FILENO)) == 1)
    {
        ...
        {
            int arg = __atoi(cmd_tkn[1]);

            if (arg == -1)
            {
                shell_printer(program_name, STDERR_FILENO);
                shell_printer(": 1: exit: Illegal number: ", STDERR_FILENO);
        ...
        {
            shell_printer("Usage: exit [status]\n", STDERR_FILENO);
            status = 2;
        }
    }
}